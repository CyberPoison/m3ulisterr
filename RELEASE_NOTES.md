...
**New: `get_vod_availability` - tells a client whether a movie has any cached candidate (used by Decypharr to hide movies that could never play):**
- **What it does:** `player_api.php?action=get_vod_availability&vod_id=<TMDB id>&username=...` asks AIOStreams for that movie (same account-language rules, year/pack guard and zero-weight-service exclusion as the real resolver in `play.php`) and answers `available: true|false`, the best rung found, and per-rung counts. It only counts candidates that are already **cached** on a debrid service and sit on the ladder 1080p x264 -> 720p x264 -> 4K x264 -> 4K x265 -> 1080p x265 -> 720p x265 (a cached release outside the ladder - SD, AV1, unknown codec - is reported as `other_cached` but does not make a title available).
- **Read-only and cheap:** it never resolves, probes or proxies a stream - one AIOStreams lookup, cached for 6 hours (`M3U_DATA_DIR/availability`, or the temp dir). An unreachable/garbled AIOStreams answers HTTP 502 with `available: null` and is never cached, so a client can tell "unknown" from "nothing available".
...
- **Verified:** `php tests/aio_availability_test.php` (22 unit checks: every ladder rung, ladder order, uncached, SD/AV1, language only in subtitles, year guard, zero-weight service), `bash tests/aio_availability_e2e.sh` (real `player_api.php` on a dev server against a mock AIOStreams: French vs English account, HTTP 502 on upstream failure and garbage, caching, `refresh=1`, failures not cached, 400 on a bad id), and against the real AIOStreams (Avengers: Endgame available with 1080p x264 best; a French title with 10 language matches but nothing cached correctly reported unavailable).
