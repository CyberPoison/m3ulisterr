...
$perPage = 100;
$maxPages = 5;
$pornDbApiKey = '0xNl0sNgCaALhoBEi1wrZRxAQTXHEBE0KX8MesTy32d9c46a';
...
// Function to fetch poster from ThePornDb REST API
function getPornDbPoster($title, $apiKey) {
    if (empty($apiKey) || empty($title)) return '';

    // URL encode the title for the query parameter
    $queryTitle = urlencode($title);
    $url = "https://api.theporndb.net/movies?q={$queryTitle}";
    ...
    // Prioritize poster over front and image
    if (!empty($json['data'][0])) {
        $movie = $json['data'][0];
        if (!empty($movie['poster'])) {
            return $movie['poster'];
        } elseif (!empty($movie['front'])) {
            return $movie['front'];
...
do {
    ...
    foreach ($response as $item) {
        ...
        $posterPath = getPornDbPoster($title, $pornDbApiKey);
        if (empty($posterPath)) {
